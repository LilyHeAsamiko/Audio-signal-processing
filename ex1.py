...
def get_ZCR(audio_window):
    ...
    zcr = 0
    N = len(_fs) - 1
    zcr = 1/N*np.sum(np.abs(np.sign(np.arange(2,N))-np.sign(np.arange(1,N))));
...
def extract_feature(_audio_filename):
    ...
    #fft_mel_bands = np.ones((1+nfft/2, nb_mel_bands))  # TODO: Q1. Replace this line to get FFT to mel band conversion matrix
    fft_mel_bands = librosa.filters.mel(_fs, nfft, nb_mel_bands, fmin=0.0, fmax=None, htk=False, norm=1)  # Hint: Check librosa.filters package
    ...
    for i in range(nb_frames):
        ...
        #_fft_en[frame_cnt, :] = np.ones((1 + nfft/2))
        _fft_en[frame_cnt, :] = fft(_fft_en[1,:])  # TODO: Q3. Calculate energy spectral density, you can use scipy.fftpack

        # calculate mel band energy (MBE)
        #_mbe[frame_cnt, :] = np.ones(nb_mel_bands)  # TODO: Q4. Calculate mel band energy
        _mbe[frame_cnt, :] = np.dot(_fft_en[frame_cnt, :],_fft_en[frame_cnt, :])
...
plot.subplot(412), plot.imshow(fft_en[:, :500].T,
                               aspect='auto', origin='lower', norm=colors.LogNorm(vmin=fft_en.min(), vmax=fft_en.max()))
plot.xlabel('Energy spectral density (ESD)')
plot.subplot(413), plot.imshow(mbe.T,
                               aspect='auto', origin='lower', norm=colors.LogNorm(vmin=mbe.min(), vmax=mbe.max()))
